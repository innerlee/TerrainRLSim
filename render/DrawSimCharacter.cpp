...
void cDrawSimCharacter::DrawCtrlInfo(const cCharController *ctrl, const cGround *ground, const tVector &offset,
                                     bool draw_3d) {
    if (ctrl != nullptr) {
...
void cDrawSimCharacter::DrawCharFeatures(const cSimCharacter &character, const cGround &ground, double marker_size,
...
void cDrawSimCharacter::DrawCtrlInfoGroundSamples(const cCharController *ctrl, const tVector &offset, bool draw_3d) {
    ...
    for (int s = 0; s < num_ground_samples; ++s) {
        ...
        // cDrawUtil::DrawPoint(sample_pos);
