...
void cShader::Bind() const { glUseProgram(mProg); }

void cShader::Unbind() const { glUseProgram(0); }

void cShader::SetUniform3(GLuint handle, const tVector &data) const {
    float f_data[] = {static_cast<float>(data[0]), static_cast<float>(data[1]), static_cast<float>(data[2])};
...
void cShader::SetUniform2(GLuint handle, const tVector &data) const {
    float f_data[] = {static_cast<float>(data[0]), static_cast<float>(data[1])};
    glProgramUniform2fv(mProg, handle, 1, f_data);
}

GLuint cShader::GetProg() const { return mProg; }

void cShader::setShaderRelativePath(std::string path) { this->mRelativePath = path; }
