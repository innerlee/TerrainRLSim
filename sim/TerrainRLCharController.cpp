...
void cTerrainRLCharController::SkipDecideAction() { mSkipDecideAction = true; }
...
int cTerrainRLCharController::GetCurrActionID() const { return mCurrAction.mID; }

int cTerrainRLCharController::GetNumGroundSamples() const { return mNumGroundSamples; }

void cTerrainRLCharController::SetNumGroundSamples(int num_ground_samples) { mNumGroundSamples = num_ground_samples; }
...
tMatrix cTerrainRLCharController::GetGroundSampleTrans() const { return mGroundSampleTrans; }
...
const Eigen::VectorXd &cTerrainRLCharController::GetTau() const { return mTau; }
...
bool cTerrainRLCharController::IsOffPolicy() const { return mIsOffPolicy; }

int cTerrainRLCharController::GetPoliActionSize() const { return GetNumActions(); }

void cTerrainRLCharController::RecordPoliState(Eigen::VectorXd &out_state) const { out_state = mPoliState; }
...
void cTerrainRLCharController::InitPoliState() { mPoliState = Eigen::VectorXd::Zero(GetPoliStateSize()); }
...
void cTerrainRLCharController::InitGroundSamples() { mGroundSamples = Eigen::VectorXd::Zero(GetNumGroundSamples()); }
...
bool cTerrainRLCharController::IsOptParam(int param_idx) const { return true; }
...
bool cTerrainRLCharController::HasGround() const { return mGround != nullptr; }
...
void cTerrainRLCharController::BuildPoliStateGround(Eigen::VectorXd &out_ground) const { out_ground = mGroundSamples; }
...
int cTerrainRLCharController::GetGroundFeatureSize() const { return GetNumGroundSamples(); }

int cTerrainRLCharController::GetPoseFeatureSize() const {
    return mChar->GetNumBodyParts() * gPosDim - 1; // -1 for root x
}

int cTerrainRLCharController::GetVelFeatureSize() const { return mChar->GetNumBodyParts() * gPosDim; }
...
#if defined(ENABLE_DEBUG_VISUALIZATION)
const cCircularBuffer<double> &cTerrainRLCharController::GetPoliValLog() const { return mPoliValLog; }
